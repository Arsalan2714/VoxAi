...
import { Coins, LogInIcon, LogOut, MessageSquare, PanelLeftIcon, PanelRight, PenSquare, Plus,  UserIcon } from "lucide-react";
...
const SideBar = () => {
  ...
  return (
    ...
    <Plus size={17}/>
              ...
              <div className="flex-1 overflow-y-auto px-2.5 pt-2 [scrollbar-width:none] [&
        ::-webkit-scrollbar]:hidden pt-5">
            {conversations.map((conv,i)=>{
              ...
              className={`flex items-center gap-2.5 cursor-pointer mb-0.5 px-3 py-2.5 rounded-[10px] border transition-colors duration-150 ${isActive ? "bg-indigo-500/10 border-indigo-500[0.18]" 
                  : "bg-transparent border-transparent"}`}>
                    <div className={`flex items-center justify-center shrink-0 w-[20px] h-[20px] rounded-lg transition-colors duration-150 ${isActive ? "bg-indigo-500/15 border-indigo-400" 
      ...
      <div className="flex flex-col h-full">
        ...
        ::-webkit-scrollbar]:hidden">
            {conversations.map((conv,i)=>{
              ...
              className={`flex items-center gap-2.5 cursor-pointer mb-0.5 px-3 py-2.5 rounded-[10px] border transition-colors duration-150 ${isActive ? "bg-indigo-500/10 border-indigo-500[0.18]" 
                  : "bg-transparent border-transparent"}`}>
                    <div className={`flex items-center justify-center shrink-0 w-[28px] h-[28px] rounded-lg transition-colors duration-150 ${isActive ? "bg-indigo-500/15 border-indigo-400" 
                  : "bg-white/[0..05] text-slate-500"}`}>
                      <MessageSquare size={13}/>
                    </div>
          ...
          <div className="px-3.5 py-3.5">
              ...
              {userData ? (
                <div className="flex items-center gap-2.5 cursor-pointer rounded-xl px-3 py-2.5 hover:bg-white/[0.5] transition-colors duration-150"> 
                    <div className="relative shrink-0">
                        {
                            (userData?.avatar && !imageError) 
                            ?
